#include "qimagegrabbericamview.h"


QImageGrabberIcamView::QImageGrabberIcamView(QObject *)
{
...
void QImageGrabberIcamView::connectToHost(QHostAddress address, quint16 port)
{
    socket.connectToHost(address, port, QUdpSocket::ReadWrite);
}


void QImageGrabberIcamView::socketStateChanged(QAbstractSocket::SocketState newState)
{
    switch(newState) {
...
void QImageGrabberIcamView::socketReadyRead()
{
...
void QImageGrabberIcamView::timeOutSlot()
{
    qWarning() << "TOUT";
    imageBuffer->close();
    sendImageRequest();
...
QStringList QImageGrabberIcamView::enumerateSources()
{
    return QStringList();
}

void QImageGrabberIcamView::newImageTimeOut()
{
    //FIXME
    sendImageRequest();
}
